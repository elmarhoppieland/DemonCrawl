...
class_name DebugToolEmblemDetails
...
@export var emblem: EmblemData :
	set(value):
		...
		if value:
			...
			_tooltip_grabber.max_line_length = EmblemObject.MAX_LINE_LENGTH
...
func _ready() -> void:
	Codex.emblems_changed.connect(update_count_label)
...
func update_count_label() -> void:
	_current_count_label.text = "In Inventory: %d" % Codex.get_emblems(emblem)
...
func _on_bind_add_to_inventory_button_pressed() -> void:
	_get_overlay().bind_action(Codex.gain_emblem.bind(emblem))
