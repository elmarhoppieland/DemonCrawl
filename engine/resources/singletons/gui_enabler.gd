...
static func get_gui_list() -> PackedStringArray:
	...
	var state := load("res://engine/resources/singletons/gui_layer.tscn").get_state() as SceneState
...
static func get_gui_branch(gui_name: String) -> Node:
	var instance := load("res://engine/resources/singletons/gui_layer.tscn").instantiate() as Node
