...
func _spawn() -> void:
	...
	var item := get_quest().get_item_pool().create_filter().filter_tag("_targeted").disallow_all_types().allow_type(Item.Type.MAGIC).set_allow_items_in_inventory().get_random_item().create()
