...
func show_rewards() -> void:
	...
	var types := Board.get_reward_types()
...
func get_score_reward(type: StringName) -> ScoreReward:
	match type:
		"victory":
			return ScoreReward.new(ceili(Board.board_3bv * 1.4))
		"flagless":
			return ScoreReward.new(ceili(Board.board_3bv * 0.7))
		"untouchable":
			return ScoreReward.new(ceili(Board.board_3bv * 0.35))
		"thrifty":
			var specials := 0
			...
			return ScoreReward.new(ceili(Board.board_3bv * specials * 0.35 / 6))
		"charitable":
			...
			reward.cap(ceili(Board.board_3bv * 0.7))
			return reward
		"heartless":
			var reward := ScoreReward.new(0)
			if Stats.life < Stats.max_life:
				for cell in Board.get_cells():
					if cell.cell_object is CellHeart:
						reward.add(5)
